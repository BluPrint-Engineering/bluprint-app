...
- The mount imposes four things that break at runtime if removed; they are listed in `.claude/rules/api.md` (`bodyParser: false`, the global `AuthGuard` with `@AllowAnonymous()` as the only opt-out, `disableTrustedOriginsCors: true`, and auth routes bypassing Nest's filter and pipe, which is why their errors are rewritten by wrapping the handler: [0047](0047-errors-are-rfc-9457-problem-details.md)).
- `trustedOrigins` only checks requests that **carry a cookie** (the shape of a CSRF attack), so a new client's first call seems to pass and later ones fail. With a cookie, a foreign `Origin` gets 403 `INVALID_ORIGIN`, and `Origin: null` (a sandboxed iframe, or a non-browser client with no header set) gets 403 `MISSING_OR_NULL_ORIGIN`. Reaching the docs at `http://localhost:5173/api/docs` avoids this entirely: same origin as the web app, so the browser sets `Origin` itself on every Try it out call.
