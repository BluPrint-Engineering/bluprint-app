...
- **Controller → service → repository.** The controller declares the route, validates through a DTO and calls the service. The service holds the logic, never touches `Request`/`Response`/Express, never injects `DATABASE` and never calls Drizzle. Database access lives in an `@Injectable()` `<feature>.repository.ts` that injects `TransactionHost<DatabaseAdapter>` and queries `this.txHost.tx`, never a stored database handle; a service method with more than one write is `@Transactional()` ([0051](../../docs/adr/0051-transaction-aware-repositories-via-cls.md)). Another feature's repository comes from importing its module. `apps/api/src/projects/` is the worked example.
- **A type on a decorated parameter is imported with `type`** (`import { Session, type UserSession }`): `db:seed` runs under Bun, which otherwise emits it as a value and fails at load ([0051](../../docs/adr/0051-transaction-aware-repositories-via-cls.md)). A DTO stays a value import.
