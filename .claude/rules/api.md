...
- **Every controller method declares a response DTO** with `@ZodSerializerDto(SomeDto)` (or `[SomeDto]`), even a route with no input. A `z.iso.datetime()` field needs the service to return a string, not a `Date` ([0004](../../docs/adr/0004-response-dto-on-every-route.md)). `apps/api/src/projects/` is the worked example.
...
- **The error contract is `AllExceptionsFilter`**, registered once in `app.ts`. Removing it breaks `{"error": "..."}` silently; the Bruno "Unknown route" request is the first to catch it.
...
- **Auth routes bypass Nest**: they skip `AllExceptionsFilter` and `ZodValidationPipe`, so their error body is Better Auth's, not `{"error": "..."}` ([0010](../../docs/adr/0010-self-hosted-better-auth.md)).
- **A new route enters the Bruno collection** (`apps/api/bruno/`) in the same PR, with the `/api` prefix in its path.
